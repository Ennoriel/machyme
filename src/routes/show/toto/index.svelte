<script>
    ...
    onMount(() => {
        intervalId = setInterval(() => x = x + 1, delay);
        visible = true;
        loaded = true;
    })
...
<div class="commands">
    ...
    {#if loaded}
        <ColorPicker bind:hex={c1} components={{...CircleVariant, input: InputColorPicker}} label="Start color" />
        <ColorPicker bind:hex={c2} components={{...CircleVariant, input: InputColorPicker}} label="End color"/>
    {:else}
        <div class="input-like" style:width="140px"/>
        <div class="input-like" style:width="140px"/>
    {/if}

    <button on:click={restartAnimation}>
        Restart animation
    </button>
</div>
